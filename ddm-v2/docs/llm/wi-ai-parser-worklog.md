...
| S-5 | **Medium ⬆ 2026-08-23 升級：不只是「宣告了卻不執行」** | `routing.ROUTING_REASONS` 定義後生產端零引用，`compute_routing` 無條件 `reasons.extend(plan.unresolved)`。**新查證**：`PlannerOutput.unresolved` 是**模型可控的 `list[str]`**，`sanitize_planner_output` 第一行 `unresolved = list(output.unresolved)` 就逐字流進 `plan.unresolved` → `routing_reasons` → `ai_parse_runs` → API 回應。協調者實測：`unresolved=['MODEL_WROTE_THIS_user:pass@host']` **一字未改**地流過全程。⚠️ 這條**早就開著**，不是 ADR-033 P1 的旁通道新開的——P1 那條刻意不走 reasons 的決定仍然對，但當時給的理由（「會**開新的**注入面」）是錯的，已更正 | **升級的理由**：P1 的旁通道靠「reason 前綴切割」（`r.split(":", 1)[0]`）擋住模型字串，資安席驗證那道切割**可靠**（含冒號的 payload 也逃不出來）——但它成立的唯一理由是「每個 reason 都以字面前綴開頭」，而**沒有任何東西強制這件事**。哪天有人寫 `f"{some_var}:..."`，防線靜默失效。**把 `ROUTING_REASONS` 落地成真正的過濾器**（`compute_routing` 把非白名單項統一映射成 `planner_unresolved`）能一次修好兩件事：關掉 `output.unresolved` 這條既有通道，並讓前綴切割**從慣例變成被強制的不變式**。⚠️ 動的是 `compute_routing` 的行為、會改變既有 reason 的值域，屬獨立切片，P1 不做 | `routing.ROUTING_REASONS` frozenset 定義後全 repo **零引用**；`compute_routing` 無條件 `reasons.extend(plan.unresolved)`，模型可控字串直接成為 `routing_reasons` 並持久化。方向保守（unresolved 非空必擋 auto），不構成權限提升，但宣告了卻不執行的白名單會誤導讀者 | 要嘛落地過濾、要嘛刪掉常數；屬清理，非阻擋 |
...
| S-12 | **Medium（功能面）／Low（資安面）** | **`base_url` 帶 userinfo 時，httpx 會用 userinfo 生成 `Authorization: Basic` 並蓋掉 `llm_client` 設的 Bearer**（httpx 0.28.1 實測，**覆蓋發生在 send 階段，`build_request` 看不到**——協調者用 `build_request` 驗過兩次都得到「Bearer 活著」的假結論，起真伺服器才現形）。兩個後果：**(a) 功能面（較嚴重）**：那種設定下 **api_key 根本沒送出去**，症狀是 401，而人會盯著 api_key 檢查半天因為它看起來是對的；**(b) 資安面**：憑證以 **base64** 形式進入回應／日誌，**明文 sentinel 掃不到、`_redact_secrets` 也不命中**——T-12 剛建的遮蔽對這種設定是漏的 | T-12 **未實作也未測試**這層防護，因此**不宣稱有防護**（CI_GATES 硬性規則 9）。修法：把 userinfo 的 base64 衍生形納入 `secrets`；或更根本地——`llm_client` 偵測到 `base_url` 含 userinfo 就 fail-loud（那本來就是設定錯誤，Bearer 與 Basic 並存沒有合理用途）。**建議選後者**，理由三條（複審補強）：⑴ **字面遮蔽對衍生形是打不贏的軍備競賽**——base64 有 padded／unpadded／URL-safe 變體，值還可能被伺服器拆行或以 `\uXXXX` 逸出形式回傳，`str.replace` 完全不命中，列舉編碼永遠慢一步；⑵ **就算列舉全了也沒用**——對方已經有那把 key（是我們送過去的），`_redact_secrets` 保護的是「**已入版控的產物**不要夾帶憑證」，防的是「回聲式 proxy／debug endpoint 不小心把 header 抄回來」這種**意外**、不是攻擊，而意外反射幾乎必然是**原樣**反射，那已經被現行實作接住；⑶ fail-loud 是**單一邊界的一次檢查**，同時消掉功能面的坑，且與本 repo 既有的兩個立場一致（spec §7.1 只允許私有 URL、`base_url` 一律不進報告）。⚠️ **檢查點放在 settings／client 建構時**，讓它在**啟動**就炸而不是跑到一半才炸（一輪評測 25 分鐘，中途失敗代價高）。⚠️ 這是**行為變更**：現在用 userinfo 的設定會直接壞掉。可接受——`DDM_LLM_API_KEY` 是支援路徑，userinfo-in-URL 本來就是 RFC 上不建議的寫法，影響面接近零。寫在這裡免得日後被當成迴歸 |
| T-16 | — | **D3 的「role.text 必須是原文字面子字串」不變式有兩個已知脆弱點**（P1 實作時查證，全 gold 掃描確認目前只影響 1 處）：⑴ **大小寫**——`normalize()` 會小寫化，而 `g01` 的 `object.text = "DIMM"` 對 `normalized_text = "拿起dimm"` **不成立**（全 55 案唯一一處）。實機上模型看到的是 normalized 文字、傾向照抄小寫，所以影響面待觀察期量；但任何沿用原文大小寫的英文片語都會被剝。⑵ **原文缺字**——見 T-14 的 `g32`（少一個「並」）。兩者同屬「不變式假設原文用字完整且大小寫一致」 | 修法有三條路，各有代價：改 `normalize` 語意（影響面最大，會動到既有 offset 與 evidence 比對）／不變式改用 casefold 比對（只解⑴）／在剝除時記名而非靜默（保守，但不解決根因）。**✅ 2026-08-23 觀察期已量到，可裁決**：`role_text_not_in_source` 全 55 案出現 **4 處**，加裝旁通道後拿到實際被剝的文字，分兩類——**⑴ 佔位字（2 處，剝對了）**：`g37` from_location=「未指定」、`g46` tool=「未指定工具」，**正是規則 4 明文禁止的**；**⑵ 壓縮式改寫（2 處，可議）**：`g46` process_kind=「鎖附固定並確認到位」（原文「右手並鎖附固定並確認**螺絲**到位」）、`g51`=「按壓到定位」（原文「按壓 **dimm 卡扣**到定位」）——字都在原文、順序也對，**但不連續**。代價明確：`process_kind` 是字典比對的查詢字串，被剝後退回 evidence 整句＝**精度掉但不失敗**。**裁決建議：不弱化不變式，改在 P2 的 prompt 教「role 的 text 必須是原文的連續片段，照抄不要改寫」。** 理由：模型壓縮改寫是因為 prompt 從沒講過這件事；為遷就模型而弱化一個**可機械驗證**的不變式，等於放棄 D3 用它取代 `status` 自陳的全部理由。**弱化不變式不可逆，教 prompt 可量測。**（子序列比對這條路其實仍擋得掉「未指定」，但一般情況太弱——`螺絲` 兩字幾乎能子序列匹配任何句子。）⚠️ 原本的大小寫脆弱點（`g01` 的 `DIMM` vs `dimm`）在這輪**沒有實際發生**——模型看到的是 normalized 文字、傾向照抄小寫，與當初的推測一致。原記錄：**觀察期先量實際發生率再決定**——若 `role_text_not_in_source` 幾乎不出現，這票可以一直記著不做 |
| T-15 | — | **initial 那次的模型回應留不下來**：adapter 只把 retry 那次掛上 `PlannerError.raw`，所以 `planner_raw_rejected` 只有第二次的內容。若模型兩次輸出形狀不同（例如第一次切分對、retry 反而被錯誤訊息帶偏），P1 觀察期會看不到第一次 | 要留得動與 `wi_ai_service` 共用的契約，超出 T-12 範圍。已寫進報告 `note` 與 README |
...
**⚠️ 自我揭露的界線（實作席自己劃的，比本節其他結論更該記住）**：自報「未保證清單」三輪
都準，但**它從來沒抓到這輪的兩個 P1**（F1 過濾不是驗證、G1 只認第一個賦值）——那兩處
實作席 mutate 過、也綠了，問題出在 **mutation 本身沒觸到真正的漏法**。

> **自我揭露答得出「我沒驗什麼」，答不出「我的驗法本身有盲點」。**

所以**自報清單不能取代獨立複審**。兩者守的是不同的失效模式：前者守「已知的缺口」，
後者守「不知道自己不知道的缺口」。把前者當成後者的替代品，是這一節最容易被誤讀的地方。

**紀律第 7 條的重點在後半句**：實作席兩次還原出錯（`_contract_freeze_v1.py` 一次、
`routing.py` 一次），**都是靠事後 `git diff --stat` 與 grep 計數才發現，不是靠記得規則**。
所以「還原後驗證 diff 為空」比「還原用 `git checkout`」重要——**「還原成功」必須被驗證，
不能被假設**。規則防不了手滑，驗證可以。

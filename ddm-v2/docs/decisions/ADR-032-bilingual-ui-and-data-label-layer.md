...
**呈現介面：本 ADR 不定案**（P6）。傾向「主數據管理」頁新增一個分頁而非獨立側欄項——
理由是側欄總數是 ADR-021／024 反覆爭取的稀缺資源，為一個過渡性工作清單再加一項不划算。
但這要對照 v3 母版與 User 的實際使用習慣，實作時再定。
...
| **P6** | **待審清單的呈現位置**：獨立側欄項 vs 「主數據管理」加分頁 vs 兩個既有頁各自顯示自己的待審 | 傾向「主數據管理」加分頁（側欄項數是 ADR-021／024 反覆爭取的稀缺資源）。但清單同時涵蓋字典（admin 入口）與主數據（analyst 入口）兩類物件，放哪邊都有一半是「別人家的東西」。實作時對照 v3 與使用習慣再定 |

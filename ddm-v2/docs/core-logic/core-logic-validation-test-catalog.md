...
### J. 整表 SIMO / frequency（ADR-020，2026-07-13：SIMO 標記列貢獻 0）
| 測試 | 為何重要 |
|------|----------|
| 56 + 0 + 0 = 56 | **SIMO 標記列（simo_group_id 非空）貢獻 0**，時間由未標記主列吸收（對齊 v3 認證語義；舊「群組取 max=85」廢止）。算錯會高估雙手同時動作的工時 |
| 單獨標記列（無同組夥伴）= 0 | 舊群組 max 語義會全額計入——ADR-020 分歧實例 (a) |
| 主列未標記全額計入（28×3=84） | freq 乘算不受 SIMO 語義影響 |
| 混合 28 + 0 + 29 = 57 | 主列＋標記列＋一般列的總計口徑 |

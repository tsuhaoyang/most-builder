...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/62d5bebb…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/89add432…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/310981d5…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
...
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/62d5bebb…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/89add432…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/310981d5…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
6. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
7. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【雙手接觸×16dimm卡槽的左右卡扣按壓×16並確認卡扣按壓規定位置】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/6e6f8d3d…`（rows[0].sub_activity；1 cycle；v3）），預設依此；不同意再改。
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/9569aeaf…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b66a74ed…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/2ea27e92…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/55e93ed7…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/b91e9fca…`（rows[1].sub_activity；1 cycle；v3）；`motion_modules/ea8ebe0c…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【雙手接觸dimm壓合治具推至規定位置】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/62d5bebb…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b7eefdb5…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/8028b635…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：GM（一般移動） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. 取移建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/62d5bebb…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b7eefdb5…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/8028b635…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 CM cycle（G 與 M 各取值）**；不同意再改成 acquire + controlled_move 兩個 action（hint 是證據不是判決，可推翻）。
5. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
6. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
7. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【雙手接觸dimm壓合治具拉至規定位置】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/437a60f0…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/b91e9fca…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/3725c049…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：GM（一般移動） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. 取移建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/437a60f0…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/b91e9fca…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/3725c049…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 CM cycle（G 與 M 各取值）**；不同意再改成 acquire + controlled_move 兩個 action（hint 是證據不是判決，可推翻）。
5. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
6. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
7. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【雙手接觸dimm卡槽的左右卡扣推至規定位置並確認到位】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/ab031277…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/b55f8099…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
5. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【右手抓握風槍按動按鈕並吹風清潔dimm卡槽】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/3c2d739e…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/76d8c950…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/39a7b9db…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【雙手接觸dimm卡槽的左右卡扣按壓並確認卡扣按壓規定位置】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_modules/45915347…`（name_zh (category=wi-template)；1 cycle；v3）；`motion_module_versions/fc8b5371…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/f495f90c…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【右手抓握風槍按動按鈕並吹風清潔dimm】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/687ed233…`（rows[4].sub_activity；1 cycle；v3）；`motion_module_versions/83f8be04…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/7b395f7d…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` move_place：【雙手重新抓握主機板放至潔淨棚的工作臺】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/47007c24…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/fb1bde71…`（rows[3].sub_activity；1 cycle；v3）；`motion_modules/d4106090…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. P 方向數：P 方向數（IE 情境規則 D3-017）：賓語屬**盤面類**（流水線/工作台/垃圾桶/料盒/材料盒/料架）→ 無方向，已套用 `p_place_none`。請確認
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/47007c24…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/fb1bde71…`（rows[3].sub_activity；1 cycle；v3）；`motion_modules/d4106090…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. SIMO/雙手：左右手是否同時動作？是否需標 hand 與 SIMO 群組？（現行 plan 契約未表達 SIMO——若需要請註記）
6. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【左手抓握主機板的包裝袋去除】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/549e17e8…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/fb1bde71…`（rows[1].sub_activity；1 cycle；v3）；`motion_modules/8a2348c0…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/1ec11b2a…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/687ed233…`（rows[2].sub_activity；1 cycle；v3）；`motion_modules/0c944bee…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/61b92e7e…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/a6e06c10…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）
3. P 方向數：P 方向數（IE 情境規則 D3-017）：賓語屬**機構件類**（治具/卡槽/機箱/接頭/點位）→ 必對準，已預設 `p_place_single`。**方向數預設一種，不對請改**（p_place_multi 多種方向／p_place_none 無方向）
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/61b92e7e…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/a6e06c10…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
6. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【右手從dimm材料盒拿取label貼附至主機板】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/2a1ba08c…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/4495bd92…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/d8232ea2…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/2a1ba08c…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/4495bd92…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/d8232ea2…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【右手接觸dimm壓合治具的底板拉至對應的點位】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/40bb7624…`（rows[2].sub_activity；1 cycle；v3）；`motion_module_versions/739d25e9…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/9af1d144…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：GM（一般移動） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. 取移建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/40bb7624…`（rows[2].sub_activity；1 cycle；v3）；`motion_module_versions/739d25e9…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/9af1d144…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 CM cycle（G 與 M 各取值）**；不同意再改成 acquire + controlled_move 兩個 action（hint 是證據不是判決，可推翻）。
5. 工具持有：工具是否跨動作持有？若是，後續動作 G 應留空並在 dependencies 標 tool_held_for。
6. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/687ed233…`（rows[3].sub_activity；1 cycle；v3）；`motion_module_versions/87f0fddc…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/ef3a2924…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` controlled_move：【右手抓握假dimm的包裝袋撕除】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/7fc58b77…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b8e3690d…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/2b73646b…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 controlled_move，對嗎？若對，core 參數 M（控制移動） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → CM（控制移動），請確認】
3. TMU=0.0：此句未述距離，**TMU=0.0 非真值**（引擎口徑：距離未述＝0cm、M 階梯 0→0；linker 只掛 core 參數，伴隨 slot 未填）——complete 是結構完成度不是 TMU 可信度。**請補距離（改 plan/cycle 後 `--recompile` 重算）或判定句子資訊不足**（轉正時顯式寫 expected_incomplete_reason；空殼守門要求 total_tmu > 0，原樣轉正會被擋）。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` move_place：【左手重新抓握主機板的包裝袋放至料架】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/37b5e479…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/fb1bde71…`（rows[2].sub_activity；1 cycle；v3）；`motion_modules/909142f5…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. P 方向數：P 方向數（IE 情境規則 D3-017）：賓語屬**盤面類**（流水線/工作台/垃圾桶/料盒/材料盒/料架）→ 無方向，已套用 `p_place_none`。請確認
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/37b5e479…`（rows[0].sub_activity；1 cycle；v3）；`motion_module_versions/fb1bde71…`（rows[2].sub_activity；1 cycle；v3）；`motion_modules/909142f5…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` move_place：【左手抓握假dimm的包裝袋丟至垃圾桶】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/0be9daab…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/9a0c3806…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/c8728419…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/0be9daab…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/9a0c3806…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/c8728419…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
4. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？
...
**預測 action 數**：1
**預測切分（evidence 以【】標在正規化原文上）**：
- `a1` move_place：【左手抓握dimm材料盒放至規定位置處】
...
**IE 請回答**：
1. 切分：預測 action 數 = 1。本句實際應拆成幾個 action？若不同，請在 plan.actions 增列並各給 evidence（原文子字串與 offset）。【v3 結構】IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/687ed233…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b7ba9e33…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/c1fde19a…`（name_zh (category=action)；1 cycle；v3）），預設依此；不同意再改。
2. 判型（a1）：預測為 move_place，對嗎？若對，core 參數 P（放置） 的 option code 是什麼？（預標註無候選時請直接填）【第三輪判型已修正：未定（composite_unknown） → GM（一般移動），請確認】
3. P 方向數：P 方向數（IE 情境規則 D3-017）：賓語**不屬機構件/盤面名單**，判不出情境——暫用預設 `p_place_single`（方向數預設一種）。**請 IE 裁決**（single/multi/none）
4. 取放建模（確認題）：v3 結構顯示 IE 當初把這句建為**單一 cycle**（證據：`motion_module_versions/687ed233…`（rows[1].sub_activity；1 cycle；v3）；`motion_module_versions/b7ba9e33…`（rows[0].sub_activity；1 cycle；v3）；`motion_modules/c1fde19a…`（name_zh (category=action)；1 cycle；v3））——**預設建成單一 GM cycle（G 與 P 各取值）**；不同意再改成 acquire + move_place 兩個 action（hint 是證據不是判決，可推翻）。
5. routing：預測 routing_status = review（理由：無）。核准後應維持這個值嗎？

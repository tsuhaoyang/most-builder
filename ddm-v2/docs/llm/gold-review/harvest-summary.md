...
|---|---|---|
| GM（move_place） | 3 | 2 |
| CM（controlled_move） | 0 | 5 |
| 未定（composite_unknown） | 44 | 40 |

判型變更 **5 筆**（草稿帶 `typing_changed_by_verb_lexicon`＋`typing_change` 舊/新值；覆核表逐筆標「判型已由動詞字典修正，請確認」）：

- `d004_51518399`：未定（composite_unknown） → CM（控制移動）——「貼附Label到主板規定位置處」
- `d008_1dd7c1d5`：GM（一般移動） → CM（控制移動）——「按壓功能測試治具」
- `d011_6be614c5`：未定（composite_unknown） → CM（控制移動）——「按壓 DIMM 卡扣到定位」
- `d029_7f085e02`：未定（composite_unknown） → CM（控制移動）——「撕除螢幕保護膜」
- `d035_2e7b2e5a`：未定（composite_unknown） → CM（控制移動）——「按壓/按鈕」
...
- **句面動詞（部分）未登記——卡 `?` 動詞，IE 裁決後可解**：14 筆
  ...
  - `d007_35372a96`（未登記：對準）：「拿取排線並對準接頭」
  - `d015_0e83128f`（未登記：下壓）：「下壓 CPU 拉桿鎖定」
  - `d017_cc6c7d56`（未登記：貼上）：「貼上序號標籤」
  - `d019_c82940ca`（未登記：對準）：「精密對準裝配」
  - `d021_1e0d9e15`（未登記：熱壓）：「熱壓導熱膠固化」
  - `d022_e552df5e`（未登記：旋緊）：「旋緊天線接頭」
  - `d023_fe5df381`（未登記：整理）：「整理機殼內線材」
  - `d028_8b6e6aab`（未登記：擦拭）：「擦拭外殼指紋」
  - `d030_553bb598`（未登記：撕開）：「撕開/移除」
  - `d033_debe277a`（未登記：掃描）：「掃描條碼建檔」
  - `d034_ff7166e0`（未登記：掃描）：「掃描/檢查」
  - `d036_00104439`（未登記：按下）：「按下電源測試按鈕」
  - `d047_1a9be08f`（未登記：壓合）：「壓合卡扣」
- **已登記動詞僅 G 或 P 單獨——單一動詞不足以定型**：20 筆
  - `d005_28f9ed7e`：「拿取螺絲 x1」
  - `d010_37fbd2a6`：「放置散熱片於 CPU 上」
  - `d012_0461f75d`：「拿取 M.2 SSD」
  - `d013_ac155900`：「拿取 DIMM 記憶體模組」
  - `d014_b2618d31`：「小範圍拿取(≤50cm)」
  - `d018_86a61399`：「自料盒拿取主機板」
  - `d024_e55c3e1c`：「放置擋板至機殼後方」
  - `d025_9b7bc11d`：「放置成品入緩衝棧板」
  - `d026_e0c7f95c`：「放置主機板入機殼」
  - `d027_901d1623`：「放置」
  - `d032_60223e3d`：「插入/組裝」
  - `d037_df2af257`：「拿取風扇模組」
  - `d038_3d3c5d8f`：「拿取顯示卡」
  - `d039_b3fe9873`：「拿取電源供應器」
  - `d040_0bc1c188`：「拿取防靜電袋」
  - `d041_bbd33b62`：「拿取出貨外箱」
  - `d042_76590adb`：「拿取側板」
  - `d043_e9064034`：「拿取保護泡棉」
  - `d044_e2d59bc4`：「拿取」
  - `d046_8ef772ab`：「左手從螺絲料盒拿取螺絲」
- **句面無動詞面命中——需 IE 改 plan／補描述（非同義詞可解）**：5 筆
  - `d006_8dfafd2e`：「折合上蓋扣合」
  - `d016_995f5d45`：「貼標籤」
  - `d020_850cc7ef`：「移動/走步」
  - `d031_21ccfce2`：「插接線材」
  - `d045_bac1cab6`：「拆箱取件」
...
- `worksheet:55555555-5555-5555-5555-555555555555`：d005_28f9ed7e、d006_8dfafd2e、d007_35372a96、d008_1dd7c1d5、d010_37fbd2a6、d011_6be614c5、d012_0461f75d、d013_ac155900、d015_0e83128f、d017_cc6c7d56、d018_86a61399、d021_1e0d9e15、d022_e552df5e、d023_fe5df381、d024_e55c3e1c、d025_9b7bc11d、d026_e0c7f95c、d028_8b6e6aab、d029_7f085e02、d033_debe277a、d036_00104439、d037_df2af257、d038_3d3c5d8f、d039_b3fe9873、d040_0bc1c188、d041_bbd33b62、d042_76590adb、d043_e9064034

其餘 19 個 component 各只含 1 筆草稿。
...
- `d007_35372a96`（no_v3_structure_signal）：「拿取排線並對準接頭」
...
套用 **18 筆**（草稿帶 `ie_review` 區塊；配對鍵＝normalized_text sha256 前 8 碼，與流水號無關）：`d001_d0350279`、`d002_6678c378`、`d003_633b52bb`、`d004_51518399`、`d005_28f9ed7e`、`d006_8dfafd2e`、`d007_35372a96`、`d008_1dd7c1d5`、`d009_4eb2b2e6`、`d010_37fbd2a6`、`d011_6be614c5`、`d012_0461f75d`、`d013_ac155900`、`d014_b2618d31`、`d015_0e83128f`、`d016_995f5d45`、`d029_7f085e02`、`d035_2e7b2e5a`

stale：0 筆（所有覆核狀態都配對到內容未變的草稿）。

已轉正（promoted）entry：**44 筆**——跳過合併（句子已在 tests/gold/wi_plans/，不再產草稿；entry 保留為轉正軌跡）：

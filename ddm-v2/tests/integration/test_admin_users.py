...
async def test_users_lifecycle_and_guards(client):
    ...
    r = await client.post("/api/v2/admin/users", json={"employee_no": emp, "display_name": "UT", "roles": ["IE"]})
    assert r.status_code == 200 and "IE" in r.json()["roles"]
    r = await client.patch(f"/api/v2/admin/users/{emp}", json={"roles": ["IE", "manager"]})
    assert "manager" in r.json()["roles"]
    assert (await client.patch(f"/api/v2/admin/users/{emp}", json={"roles": ["wizard"]})).status_code == 422
    # 不可移除自己的 admin / 停用自己
    assert (await client.patch("/api/v2/admin/users/IEC141289", json={"roles": ["IE"]})).status_code == 409
    assert (await client.patch("/api/v2/admin/users/IEC141289", json={"is_active": False})).status_code == 409
    assert (await client.patch("/api/v2/admin/users/NOPE_NOBODY", json={"roles": ["IE"]})).status_code == 404

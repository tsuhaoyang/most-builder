...
import type { NlBadgeState } from './nlDraft'
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="inline-flex items-center overflow-hidden rounded-md border border-slate-300 bg-white shadow-sm">
          <button
            ...
            aria-label={t('workbench.slot.repeatDecreaseAria', { label })}
          >−</button>
          <input
            type="number"
            min={1}
          ...
          <button
            ...
            aria-label={t('workbench.slot.repeatIncreaseAria', { label })}
          >+</button>
        </div>
        <span className="text-sm text-slate-400">{t('workbench.slot.repeatHint')}</span>
      </div>
    </div>
  )
}
...
interface ContextItem { type: 'context'; key: string; labelKey: string; nvKey?: NvKey; kind?: string }
interface SlotItem { type: 'slot'; key: string; slotKey: SlotKey; param: string; captionKey: string }
type BuilderItem = ContextItem | SlotItem

const GM_ITEMS: BuilderItem[] = [
  { type: 'context', key: 'hand', labelKey: 'hand' },
  { type: 'context', key: 'from', labelKey: 'from', nvKey: 'from', kind: 'from' },
  { type: 'slot', key: 'A1', slotKey: 'a0', param: 'A', captionKey: 'aGrasp' },
  { type: 'slot', key: 'B1', slotKey: 'b1', param: 'B', captionKey: 'bGrasp' },
  { type: 'slot', key: 'G', slotKey: 'g', param: 'G', captionKey: 'g' },
  { type: 'context', key: 'target', labelKey: 'target', nvKey: 'obj', kind: 'object' },
  { type: 'context', key: 'component', labelKey: 'component', nvKey: 'component', kind: 'component' },
  { type: 'slot', key: 'A2', slotKey: 'a3', param: 'A', captionKey: 'aMove' },
  { type: 'slot', key: 'B2', slotKey: 'b4', param: 'B', captionKey: 'bMove' },
  { type: 'slot', key: 'P', slotKey: 'p', param: 'P', captionKey: 'p' },
  { type: 'context', key: 'to', labelKey: 'to', nvKey: 'to', kind: 'to' },
  { type: 'slot', key: 'A3', slotKey: 'a6', param: 'A', captionKey: 'aReturn' },
]
...
const CM_ITEMS: BuilderItem[] = [
  ...
  { type: 'context', key: 'target', labelKey: 'target', nvKey: 'obj', kind: 'object' },
  { type: 'context', key: 'component', labelKey: 'component', nvKey: 'component', kind: 'component' },
  { type: 'slot', key: 'M', slotKey: 'm', param: 'M', captionKey: 'm' },
  { type: 'context', key: 'to', labelKey: 'to', nvKey: 'to', kind: 'to' },
  { type: 'slot', key: 'X', slotKey: 'x', param: 'X', captionKey: 'x' },
  { type: 'slot', key: 'I', slotKey: 'i', param: 'I', captionKey: 'i' },
  // 「哪裡」沿用 to 詞彙庫（位置類詞彙），僅入敘述/顯示
  { type: 'context', key: 'where', labelKey: 'where', nvKey: 'where', kind: 'to' },
  { type: 'slot', key: 'A3', slotKey: 'a6', param: 'A', captionKey: 'aReturn' },
]

// 格位 modal 抬頭與底部計算方式說明的 i18n key（ADR-022 批次 C；文案照 v3
// SlotModal.vue modal-formula）：字面值在 shared/i18n/resources/*.ts。
const slotModalTitleKey = (slotKey: SlotKey) => `workbench.slot.modalTitle.${slotKey}`
const slotFormulaKey = (slotKey: SlotKey) => `workbench.slot.formula.${slotKey}`
...
// ── AI badge（F-05 四態）樣式 ──────────────────────────────────────────────────
// 吃語言無關的態，不吃顯示文字：原本是 `text.includes('推斷')` 這種中文字面哨兵，
// 英文化之後三個分支會全部靜默落到 else（配色一律變成「明確」的藍）。顯示文字
// 由 `workbench.nl.badge.*` 提供，與配色分開。
function badgeCls(state: NlBadgeState): string {
  switch (state) {
    case 'inferred': return 'bg-emerald-50 text-emerald-600 border-emerald-200'
    case 'default': return 'bg-amber-50 text-amber-600 border-amber-200'
    case 'pending': return 'bg-red-50 text-red-500 border-red-200'
    case 'exact': return 'bg-blue-50 text-blue-600 border-blue-200'
  }
}
...
export interface SlotBuilderProps {
  ...
  slotBadges?: Record<string, NlBadgeState>
}
...
export function SlotBuilder({
  ...
  const contextBlock = (item: ContextItem) => (
    <div
      ...
      // （e2e 曾用色塊可見文字定位，抬頭文案一改就失準——b66829d 的「A · A1」踩過；
      //   抬頭現已收編回單一 key（見下方 slotBlock），但順序驗證仍走 testid 不走文案）。
    ...
    >
      <span className="text-[10px] text-slate-500 text-center mb-0.5">{t(`workbench.slot.context.${item.labelKey}`)}</span>
      <div className="w-full min-w-0">
      <ComboBox
        options={vopts(item.kind!)}
        value={cur.nv[item.nvKey!]}
        placeholder={t(`workbench.slot.context.${item.labelKey}`)}
  ...
  const handBlock = () => (
    ...
    >
      <span className="text-[10px] text-slate-500 text-center mb-0.5">{t('workbench.slot.handLabel')}</span>
      <Sel
        value={cur.handCode}
    ...
    return (
      <div key={item.key} data-testid={`flow-item-${item.key}`} className="relative flex">
        <button
          ...
          title={t(slotModalTitleKey(item.slotKey))}
        >
          <span className={`flex items-center justify-between px-2 py-1 text-[10px] font-semibold leading-none ${clr.strip}`}>
            {/* 抬頭＝單一格位鍵標籤（A1/B1/G…）。三方版本：ADR-021:53／frontend-ux-spec §2.3
                流程寫「…｜A1｜B1｜G｜…」（單標籤）；v3 SlotBlock.vue:142-143 是 code＋key
                兩個獨立 span（無分隔符）；b66829d 則發明了「A · A1」（三邊都不是）。
                2026-08-15 依 DOC_REGISTRY 權威序以 accepted ADR 收編為單一 key。
                參數碼仍由色帶配色（SLOT_COLORS[param]）與說明文字（caption 含「(A)」等）承載。
                data-testid=slot-key：e2e 對抬頭做精確比對用的錨點。不能改比對外層色帶
                span——st.filled 時那層還含 ✕ 清除鈕，textContent 會變成「A1✕」。 */}
            <span data-testid="slot-key">{item.key}</span>
            {st.filled && (
              <span
                role="button"
                aria-label={t('workbench.slot.clearAria', { key: item.key })}
          ...
          <span className="flex flex-1 flex-col justify-center px-2 py-2">
            ...
            <span className="mt-1 text-[9px] leading-tight opacity-80">{t(`workbench.slot.caption.${item.captionKey}`)}</span>
          </span>
        </button>
        {badge && (
  ...
  const aBlock = (slot: ASlot, onSlot: (s: ASlot) => void, isReturn = false) => (
    <div className="space-y-3">
      <div className="text-xs font-medium text-slate-600">{t('workbench.slot.a.reachDistance')}</div>
      <ADistanceSelector
        value={slot.reach}
      ...
      {!isReturn && (['twist', 'foot'] as const).map(comp => (
        <div key={comp} className="flex items-center gap-3">
          <label className="text-xs font-medium text-slate-600 min-w-[64px]">
            {comp === 'twist' ? t('workbench.slot.a.twist') : t('workbench.slot.a.foot')}
          </label>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm text-slate-700">
            <span>{t('workbench.slot.g.title')}</span>
            <Hint tip={t('workbench.slot.g.hint')} />
          </div>
          <Sel value={cur.g} onChange={v => set({ g: v, gMod: {} })}
            opts={[{ v: '', l: t('workbench.slot.g.pick') }, ...opts.g.map(o => ({ v: o.code, l: o.label }))]}
            cls="w-full border border-slate-300 rounded-md px-3 py-2 text-sm bg-white"
          />
        </div>
        <RepeatStepper
          label={t('workbench.slot.repeatCount')}
          value={cur.gRepeat}
          onChange={next => set({ gRepeat: next })}
          ariaLabel={t('workbench.slot.gRepeatAria')}
        />
        {g?.requires_modifier && g.modifier_key && (
          <div className="space-y-2 border-t border-dashed border-slate-200 pt-4">
            <div className="text-sm text-slate-700">{t('workbench.slot.g.modifierTitle')}</div>
  ...
  const pBlock = () => (
    <div className="space-y-4">
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-sm text-slate-700">
          <span>{t('workbench.slot.p.title')}</span>
          <Hint tip={t('workbench.slot.p.hint')} />
        </div>
        <Sel value={cur.p_base} onChange={v => set({ p_base: v })}
          opts={[{ v: '', l: t('workbench.slot.p.pick') }, ...opts.p_bases.map(o => ({ v: o.code, l: o.label }))]}
          cls="w-full border border-slate-300 rounded-md px-3 py-2 text-sm bg-white"
        />
      </div>
      <div className="grid gap-2 md:grid-cols-2">
        {[0, 1].map(i => (
          ...
          opts={[
            { v: '', l: t('workbench.slot.p.addonNone') },
            ...opts.p_addons
              .filter(option => {
      ...
      <RepeatStepper
        ...
        ariaLabel={t('workbench.slot.pRepeatAria')}
      />
      {cur.p_addons.some(c => opts.p_addons.find(a => a.code === c)?.needs_precision) && (
        <div className="space-y-2 border-t border-dashed border-slate-200 pt-4">
          <div className="text-sm text-slate-700">{t('workbench.slot.p.precisionTitle')}</div>
      ...
      {cur.p_addons.some(code => (P_ADDON_MUTEX[code] ?? []).length > 0) && (
        <span className="text-[10px] text-amber-700">{t('workbench.slot.p.mutex')}</span>
      )}
    </div>
  )
  const mVerbBlock = () => {
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-base">{t(slotModalTitleKey(activeSlot))}</h3>

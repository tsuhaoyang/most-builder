...
| # | 案例 | 句子 | 命中動詞→option | 缺的物理量 | 目前狀態 | TMU=0 的 cycle | IE 裁決（補距離 cm／維持資訊不足） |
|---|---|---|---|---|---|---|---|
| 10 | `d004_51518399` | 貼附Label到主板規定位置處 | 貼附→`m_attach` | M 距離（cm） | 草稿（zero_tmu 已裁 distance_unstated；卡切分未確認不轉正） | a1：`A0 B0 G0 M0 X0 I0 A0` | （待答） |

### C. 草稿——**未裁**（1 筆；本輪不轉正，D3-021 起掛清單）

| # | 案例 | 句子 | 命中動詞→option | 缺的物理量 | 目前狀態 | TMU=0 的 cycle | IE 裁決（補距離 cm／維持資訊不足） |
|---|---|---|---|---|---|---|---|
| 11 | `d029_7f085e02` | 撕除螢幕保護膜 | 撕除→`m_teartape` | M 距離（cm） | 草稿（判型 null→CM 已確認；zero_tmu **未裁**——不轉正） | a1：`A0 B0 G0 M0 X0 I0 A0` | （待答） |

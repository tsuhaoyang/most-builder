...
7. **測試必須自足，且反向斷言必須附 mutation 證據。** 兩條都是 2026-08-12 這一輪
   ...
   - **不得依賴環境既存資料。** 不要撈「第一列」「任一筆」——自己建。CI 後端 job 的 seed
     ...
     **「既存資料」不只是「CI 有沒有種」，也包括「共用開發庫上被實機操作改過」**
     （2026-08-22 補；同一條規則的另一面）：`ddm_v2_most` 是所有人共用的開發庫，
     任何人在 UI 點一下就會改掉它的可變狀態，而**該狀態是 commit 過的、種子腳本
     還原不回來**。斷言前提必須由測試自己 arrange（寫入走 conftest 的 savepoint
     隔離，測試結束 rollback，共用庫零殘留）。已抓到的兩個實例：
     `test_me_returns_identity` 斷言「locale 未設定→回系統預設」卻直接吃 DB 現況，
     切一次語言（`PATCH /me/locale`）就常紅；i18n 覆核測試整批假設「還沒有人覆核過」
     （`reviewed == 0`／`status == 'unreviewed'`／待審清單長度＝全部候選列），
     在 D6 介面按一次「標記已覆核」就常紅——且 `dev_seed_i18n_labels.py` 明文
     「側表已有列就不重覆寫入（尊重既有覆核狀態）」，重跑種子還原不回來。
     **孿生陷阱：兩個狀態在回應上長得一樣時，測試證不到它宣稱的事**——顯式
     `locale='zh-TW'` 與「未設定→回退 zh-TW」的 `/me` 回應完全相同，所以測試要另外
     查原始欄位（`app_users.locale IS NULL`）才分得出來，並補一條非預設值的對照組。
     **「撈第一筆」的同型實例（2026-08-22）**：`test_motion_modules.py`／
     `test_motion_module_rows.py`／`test_motion_module_search_projection.py`／
     `test_wi_set.py` 的 `_get_rule_set_id()` 取 `GET /rule-sets` 的 `[0]`，實測回的是
     **V1（published 非 active）**，而同檔黃金列註明是「V2 rule-set」的值——publish
     釘到哪一版純看 tie-break。四支已改成 `GET /rule-sets/active`。
...
| 身分/RBAC | `/me` 身分；viewer 打 IE 端點→403；**locale 解析（ADR-032 D3.1）**——「未設定（`app_users.locale IS NULL`）→ 回解析後的系統預設」與「顯式存了非預設值 → 回那個值」兩條對照（少了對照組就證不到 `zh-TW` 是解析出來的而非寫死），且「未設定」由測試自己 UPDATE 成 NULL 再斷言、並回查原始欄位確認讀取端解析**不回寫**欄位（硬性規則 7 第一則） | `tests/integration/test_v2_api.py` |
...
| **錯誤契約：route 層無裸 HTTPException（ADR-034 A5／D6）** | AST 掃 `api/routes/` 全 `.py`，`raise HTTPException(...)` 命中即紅（區分真正 raise 與 docstring/註解/import 字樣）；唯一白名單＝`motion_module.py` 的 501 promote placeholder（綁「檔名+狀態碼」非行號）；後設測試斷言該白名單處仍為 501（不腐爛）；mutation 防恆綠 | `tests/unit/test_route_error_envelope_guard.py` |
...
| **對外列舉契約凍結（ADR-011）** | `nlp/contracts.py` 五個 Literal 別名（`ActionType`／`RoleStatus`／`DependencyType`／`CandidateSource`／`RoutingStatus`）、模型欄位上的 inline Literal（`SourceRef.kind`／`*.language`，共 8 欄）、`ROLE_KEYS`、`routing.ROUTING_REASONS`、`schemas/v2` 六個對外別名（`SourceLiteral`／`VocabKind`／i18n 四個），逐一比對**寫死的 v1 凍結下界**（`tests/unit/_contract_freeze_v1.py`）。判準 **基準 ⊆ 實際**：只准長不准縮，改名＝舊成員消失也紅；加成員維持綠（ADR-011 明文允許加法）。**下界不得從 `get_args()` 取**——拿被測物當合法集合來源，收窄時判準跟著縮，守衛不但擋不住還會替收窄背書（`test_prompt_few_shots` 的 dependency-type／role-key 兩條原本正是這形態，現已改為先斷言凍結下界）。**空跑防護**：另釘 `get_origin(...) is Literal` 與成員數下界（別名改成 `str` → `get_args()` 回空 tuple → 集合運算對空集合恆真；實測該 mutation 一次紅六條），並釘 AST 掃描器至少掃得到那五個已知別名。**涵蓋率**：AST 掃四個模組的模組級 Literal 別名＋反射 `contracts` 全部 BaseModel 的 Literal 欄位，掃到卻不在凍結表裡一律紅（新別名不得靜默逃過凍結），凍結表列了已不存在的項目也紅。**`blocked` ⊆ `ROUTING_REASONS`**：`_eligible_auto` 的 function-local `blocked` 字面量（AST 取，非 grep）才是真正在執行的封閉集合，宣告面漏列即紅。⚠️ **抽取器必須驗證而非過濾**——過濾掉的元素不留痕跡，數量地板數的是過濾後的殘骸，結構上不可能發現自己漏了東西（實測：`blocked` 放一個 `Name` 引用 → elts 15／抽到 14 → 全綠）；且必須**掃完整個函式收集所有綁定**，撞到第一個 `Assign` 就 return 會讓 `blocked |= {...}`／`blocked.update(...)`／條件式再賦值全部靜默（實測亦全綠）。現行判準：綁定恰好一次 → 是集合字面量 → 元素全為字串常數，三段任一不成立即**大聲失敗**。**已知天花板**：只讀得懂單一字串字面量，更複雜的寫法會被擋下而非讀懂——這是刻意的摩擦。**單向且不對稱有名字**：`_INTENTIONALLY_UNBLOCKED`（4 筆，有 `==` 防掏空常數）把「刻意不在 `blocked`」寫成可執行斷言並逐條附理由，分兩類——`fallback_rule_based`（經 `extra_reasons`）與 `composite_unknown`（經 draft issues）補進去會**改變 auto 語意**；`planner_invented_action`／`tool_state_violation` 走 `plan.unresolved`、被第一行擋得更早，屬**冗餘**——**加這條時 `template_hint` 就是紅的**（`routing.py:88` 寫進 `reasons`、`:127` 擋 auto，卻不在宣告裡），補完才綠。⚠️ **只單向**：`ROUTING_REASONS` 有而 `blocked` 沒有的四個（`composite_unknown`／`fallback_rule_based`／`planner_invented_action`／`tool_state_violation`）是**正確設計**——後兩者走 `plan.unresolved`（`contracts.py:634` 取 `r.split(":",1)[0]` 的裸前綴），被 `_eligible_auto` 第一行 `if plan.unresolved: return False` 擋得更早更硬；斷言反方向會把正確設計判成錯。**防腐爛（`actual ⊆ base`，21 例，與收窄禁令分開兩支）**：合法新增成員卻沒登記進凍結表 → 該成員從此不受保護、日後被拿掉不會紅；實測加 `RoutingStatus.deferred` 不登記 → **新守衛 2 紅、舊的 `members_only_grow` 11 支全綠**，證明舊守衛單獨存在時這個缺口會靜默打開。`MIN_*` 六個常數用 **exact parity（`==`）**：`>=` 會沉默鬆弛（表長大、常數不動 → 一次刪多個仍全綠）。⚠️ `ROUTING_REASONS` 這一項**不是**反序列化閘門（`routing_reasons` 是 JSONB `list[str]`，沒有任何地方拿它驗過），收窄壞的是**前端文案 key** 與「`blocked ⊆ 宣告` 那條守衛的判準跟著縮」。凍結表自身有規模下界（含 `FROZEN_ALIAS_SCAN_MODULES`，它是唯一被 parametrize 直接消費的表）——**已實測**：`FROZEN_ALIAS_MEMBERS` 掏空 → 5 failed **＋ 3 skipped**，那 3 skipped 正是「參數化清單一空、斷言整批消失而非變紅」的無聲失效形狀。**實測動機**：拿掉 `RoleStatus` 的 `"default"` 曾 **1385 passed 全綠**；唯一會紅的成員是偶然有 producer 的那個。**已知盲區**：`schemas/v2` 新增的模組級別名不會被自動偵測（掃描模組清單是顯式的）；`rule_set_options.vision_scope`／`kind`、`motion_module.category`／`scope` 尚未凍結（票 G-3） | `tests/unit/test_contract_enum_freeze.py`、`tests/unit/_contract_freeze_v1.py`、`tests/unit/test_prompt_few_shots.py` |
| **評測旁通道不得進生產（ADR-033 P1）** | `LLMPlannerAdapter(on_sanitize=…)` 的 callback 帶 `StripDetail`＝**被剝除的原文值**（模型可控字串：role 的 `text`／`value`／`unit`、定位不到的 evidence 片語），刻意只走 callback、不進 `reasons`／`unresolved`／`routing_reasons`，唯一合法訂閱者是 `scripts/wi_ai_eval.py`。兩條互補守衛：**(a) 行為**——真的走一次 `wi_ai_service._try_llm_plan`，斷言建構 kwargs 不含 `on_sanitize` 且該顆 adapter 的 `_on_sanitize is None`（連顯式傳 `None` 也擋：那會讓「生產不訂閱」看起來像可調參數，而它是不變式），並斷言 spy 真的被走到（否則後續全是空跑）；**(b) 靜態**——全 `src/` 掃 `on_sanitize` 字面，只放行定義站 `nlp/llm_planner.py`，涵蓋任何新開的呼叫點與 `planner._on_sanitize = f` 這種繞過建構子的寫法。空跑防護：正向控制釘 `scripts/wi_ai_eval.py` **必須**掃得到（needle 拼錯／參數改名先在這裡紅）＋掃描檔數下界（≥100，當下 138）＋放行清單過期偵測（定義站不再含該字面＝豁免過期）。⚠️ 措辭要準：被擋的是**發送**不是**收集**（`_parse_sanitize_validate` 一律建 `details`，`_observe` 在無訂閱者時 no-op）。**已知未保證**：掃描只認字面，字串拼接（`getattr(planner, "_on_" + "sanitize")`）繞得過 | `tests/unit/test_production_no_sanitize_observer.py` |
...
| **i18n 覆核側表與灌值腳本（ADR-032 D5／D6）** | DB CHECK（`source='human'` 必填 `reviewed_by`／entity_type／field／locale／source 值域）＋ UNIQUE(entity_type, scope_key, field, locale)；`upsert_review_state` get-or-create、未知 scope_key fail-closed、只存在於 draft 的 code 仍放行（D4）；sha256 過期偵測（中文改了→stale；區分 fresh machine 與 stale）；scope_key 跨 rule-set 版本共用；**灌值腳本兩條寫入路徑都驗**——測試先把 V2 的 `label_en`／`sentence_text_en` 與對應側表列清空再灌（真實 DB 早已 commit 過值，不清空等於寫入路徑根本沒被執行到：把句面寫入整段停掉照樣全綠），灌後**無條件**斷言兩欄皆非 NULL、側表 `field='label'` 與 `field='sentence'` 各一列且 `source='machine'`（句面路徑若靜默 no-op，`test_narrative_en_endpoints.py` 的「英文敘事不得殘留中文」會因 skip 條件成立而**關掉自己**）；冪等；D6 範圍不碰 published 非 active（V1 兩欄維持 NULL）；legacy_seed 只登記不覆寫既有值；未知 option code fail-loud；英文標籤在同一張表內唯一；待審清單 summary/pending 反映現況；**「還沒有人覆核過」這個基準線由測試自己 arrange**——凡斷言 `reviewed == 0`／`status == 'unreviewed'`／待審清單長度的測試，灌值前先 `DELETE FROM i18n_review_state WHERE source='human'`（`_reset_human_review_state()`），否則實機在 D6 介面按一次「標記已覆核」就整批常紅、且重跑種子還原不回來（硬性規則 7 第一則） | `tests/integration/test_i18n_review_state.py`、`tests/integration/test_i18n_routes.py`、`tests/integration/test_i18n_review_mutations.py` |
| **UI 外殼字串外部化守衛（ADR-032 R4）** | 全樹 `src/**/*.ts(x)` **註解以外**不得有中文字面值（JSX 文字節點與屬性值也算——最大宗的違規形態根本不在引號裡），外加 `toLocaleString('zh-TW')` 這類寫死語系的日期格式化（漢字掃描器對它全盲，正解是 `toLocaleString(i18n.language)`）。**兩種豁免、都必須寫在註解裡、都要寫理由**：行級 `// i18n-exempt: <理由>`（行尾或緊鄰前一行）、檔級 `i18n-exempt-file[<漢字預算>]: <理由>`（限檔頭＝第一行程式碼之前）。**兩者都只認註解裡的標記**：剝離註解後標記還在＝它躲在字串字面值裡，不予採信（否則 `const s = '中文 // i18n-exempt: 理由'` 一行字串就能消音那一行／整檔）。檔級豁免必須帶**漢字預算**並斷言該檔剝離註解後的漢字數不超過它——只准降不准升，否則豁免檔會變成永久盲區（`stale-file-exemption` 只在整份乾淨時才報，對「越長越多」全盲）。空白／純標點的理由、缺預算、標記不在檔頭，一律**不授予豁免**（那一行／那個檔的中文照樣紅）；兩者皆做**過期偵測**（指向的那行已無中文、或整檔已乾淨 → 紅），且不得並存於同一檔。注意「理由必填」只擋得住空理由——`TODO` 這種敷衍理由擋不住，理由成不成立是人工判斷關卡。守衛的地基是那支註解剝離器——它若把該擋的當註解剝掉會**安靜地永遠綠**，所以同檔內建後設測試：剝離器正確性（**字串內**的 `//`／`/* */`、網址雙斜線、樣板字面值、JSX 文字節點都必須抓到；整行／行尾／區塊／JSDoc／`{/* */}` 都不得誤報）、**regex literal 不得把剝離器帶進區塊註解狀態**（`/[/*]/` 曾讓後續三行 JSX 中文跨行蒸發且不報 desync；現以「前一個非空白字元＋同一行內閉合」辨識 regex，判錯只會把那一段當程式碼保留＝往多報掉）、行級／檔級標記躲在字串字面值裡都不予採信（否則一行字串就能消音那一行／整檔）、檔級豁免的漢字預算會擋下「豁免期間新增中文」、合成違規檔丟進**真實掃描流程**必紅（證明 R4-2 不是恆真的空清單）、排除清單比對**路徑**而非檔名（誘餌 `zh-TW.ts` 不得被整批消音）。**已知邊界**：`//` 只要不在字串／regex 裡就當行註解，所以 **JSX 文字節點裡的裸網址**（`<a>下載 https://x/y.xlsx 檔案</a>`）之後的同一行中文會被吃掉——字串裡的網址不受影響（R4-4 有測）。掃描範圍取全樹而非 diff：diff-based 要靠 base ref，`push` 到任意分支時未必存在。**純 node，不開瀏覽器也不打後端**，所以掛在 `frontend` job（`e2e` job 跑全套時會再收一次）。 | `src/frontend/e2e/i18n-hardcoded-zh.spec.ts` |
| **DSX a3 距離查詢（MVP，DSX 整合 API 契約 v2 §3.1／§6）** | `POST /api/v2/dsx/a3-distance`：正常路徑（`available:true` 帶 provisional／出處／`horizontal_cm`／`vertical_cm` 僅供參考）＋四種 `available:false` reason（`integration_disabled`／`vocab_not_mapped`／`dsx_object_not_found`／`dsx_unreachable`，含 DSX 回應畸形＝`KeyError`／`ValueError`／`JSONDecodeError` 併入 `dsx_unreachable`）；`wi_row_id` 選填——省略時純查詢不驗不寫、有值時比照 `wi_context_service._require_editable_row`（不存在→404、所屬版本非 draft→409 `VERSION_PUBLISHED`）；`require_role("analyst")`，匿名→401；同鍵 upsert（`created_by` 只記首筆、`updated_by` 每次更新）；`external_code` 合成 `to` 後綴（`SYNTHETIC_TO_SUFFIX`）送 DSX 前必須剝除；client 層總時限（`asyncio.timeout` 包住 httpx，防慢速滴流無限期佔住 DB 連線）＋ URL／request body 逐欄比對 | `tests/integration/test_dsx_a3_distance_api.py`、`tests/unit/test_dsx_service.py`、`tests/unit/test_dsx_client.py` |
| **DSX 3D 擺放介面入口（內部／工程用，iframe embed）** | `GET /api/v2/dsx/ui-url`：`current_user` 認證，未認證→401；`url` 未設定回 `null`（前端入口 disabled）。前端確認對話框（單一 WebRTC 觀看者警告）＋ iframe modal；e2e 必須 mock 掉 iframe 目標網址，不得連到真實 DSX 主機（`172.32.3.60` 非 RFC1918 私網段，且該機零認證單一觀看者，誤連會把真實使用者踢下線） | `tests/unit/test_dsx_ui_url.py`、`tests/integration/test_dsx_ui_url_api.py`、`src/frontend/e2e/ux-compliance.spec.ts`（§H-DSX3D） |
...
- **frontend**：`npm ci` → typecheck → typecheck:e2e → build → **ADR-032 R4 i18n 守衛**（`npx playwright test e2e/i18n-hardcoded-zh.spec.ts`——純 node 掃描，掛在這裡是為了讓硬編中文在最快的 job 就紅，不必等 e2e 起 DB）
...
( cd src/frontend && npm run typecheck && npm run typecheck:e2e && npm run build \
  && npx playwright test e2e/i18n-hardcoded-zh.spec.ts --reporter=line )   # 末條＝ADR-032 R4 守衛（免瀏覽器/免後端）

...
def classify_seq(norm: str, raw: str, params_hit: frozenset[str]) -> str | None:
    ...
    - ※1 名詞 GM × 動詞 CM → **CM**（動詞勝）：
      ...
      **情境限縮已解除（D3-024，2026-08-17 User 親答）**：D3-021 答②原是
      條件式裁決（「鎖附→X」限電動起子情境、手動鎖附遇到再議）；D3-024 IE
      確認**產線無手動鎖附**——「鎖附」一律當電動（X/CM），**無條件**。
      語料 4 筆句面無電動起子脈絡的「鎖附螺絲」型句（bc473698/3791550c/
      7ff8b879/323b04c1）自此在裁決字面內，不再落「再議」區。**不建情境
      旗標**（User 明選無條件裁決而非情境守門；取捨與風險註記——未來若
      出現手動鎖附工位需回頭補情境守門——記 worklog D3-024）。X/I 面自
      D3-024 起由 linker 掛進對應格（nlp/linking.py `_CM_EXTRA_SPECS`）；
      判型變更旗（typing_changed_by_verb_lexicon）未經 IE 確認仍擋轉正
      （既有守門不變）。
      **這不是全域「動詞壓名詞」**，見 ※2。
